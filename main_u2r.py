...
columns=list(X_U2R)
columns_test=list(X_U2R_test)

#Scale df using StandardScalar()
scaler1 = preprocessing.StandardScaler().fit(X_U2R)
X_U2R=scaler1.transform(X_U2R) 
# test data
scaler5 = preprocessing.StandardScaler().fit(X_U2R_test)
X_U2R_test=scaler5.transform(X_U2R_test) 

X_U2R_test = pd.DataFrame(X_U2R_test)
...
X_U2R_test = X_U2R_test[X_U2R_test.columns[newcolindex_U2R]]


#Training U2R
xu2r,yu2r=X_newU2R,Y_U2R
#xdos=xdos.values
x_testu2r,y_testu2r=X_U2R_test,Y_U2R_test
#x_testdos=x_testdos.values
y0u2r=np.ones(len(yu2r),np.int8)
y0u2r[np.where(yu2r==classes[0])]=0
y0_testu2r=np.ones(len(y_testu2r),np.int8)
y0_testu2r[np.where(y_testu2r==classes[0])]=0

autoencoder = getModel(xu2r)
history=autoencoder.fit(xu2r[np.where(y0u2r==0)],xu2r[np.where(y0u2r==0)],
               epochs=10,
                batch_size=100,
                shuffle=True,
...
print(type(X_U2R_test))

# We set the threshold equal to the training loss of the autoencoder
threshold=history.history["loss"][-1]
x_testu2r=x_testu2r.to_numpy()
testing_set_predictions=autoencoder.predict(x_testu2r)
test_losses=calculate_losses(x_testu2r,testing_set_predictions)
testing_set_predictions=np.zeros(len(test_losses))
testing_set_predictions[np.where(test_losses>threshold)]=1


accuracy=accuracy_score(y0_testu2r,testing_set_predictions)

#inline plot
c = confusion_matrix(y0_testu2r,testing_set_predictions)
plot_confusion_matrix(c,["Normal","U2R"])
